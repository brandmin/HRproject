...
public class DepartmentsDao {
	...
	public void insert(DepartmentsDto dto) {
		String sql = String.format("insert into departments values (%d, %d, '%s', %d",
				dto.getDepartment_id(), dto.getLocation_id(),
				dto.getDepartment_name(), dto.getManager_id());
		...
		try {
			while(rs.next()) {
				resultDtos.add(new DepartmentsDto(
						rs.getInt("department_id"), rs.getInt("location_id"),
